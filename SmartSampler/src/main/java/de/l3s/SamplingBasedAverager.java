...
/**
 * This is a class for efficient estimation of a statistics in a very large data set.<br> 
 * It delivers a value with a bounded error that does not exceed given value within a given confidence interval.<br> It is using a central limit theorem and Tschebyshev inequality.<br>
...
public class SamplingBasedAverager {
	 ...
	 * 
	 * @param error: maximal sampling error (0-1.0)
	 * @param confidence: minimal confidence (0-1.0)
	 */
	 ...
	 * @param error: maximal sampling error (0-1.0)
	 * @param confidence: minimal confidence (0-1.0)
	 * @param w: initial sample size and sample step.
	 */
 ...
 *  
 * @param s: sampler object, which task is to provide a value of a random object from the population
 * @return
 */
	public double randomSampling(Sampler s) {
		...
		int r2 = (int) Math.ceil(Math.log(1/(1-confidence))/Math.log(2)); //(int) Math.ceil(Math.log(1.0D / confidence) / Math.log(2D));
